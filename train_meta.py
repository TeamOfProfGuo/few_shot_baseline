...
def main(config):
    ...
    train_loader = DataLoader(train_dataset, batch_sampler=train_sampler, num_workers=4, pin_memory=True) #共200个batch
    ...
    if config.get('tval_dataset'):
        ...
        tval_loader = DataLoader(tval_dataset, batch_sampler=tval_sampler, num_workers=4, pin_memory=True)
    else:
        tval_loader = None
    ...
    val_sampler = CategoriesSampler(
            ...
            n_way, n_shot + n_query,
            ep_per_batch=4)  # 生成每个batch idx: [320] = 4*5*(1+15)
    val_loader = DataLoader(val_dataset, batch_sampler=val_sampler, num_workers=4, pin_memory=True) # 共200个batch
